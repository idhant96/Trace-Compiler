...
eval_numberDeclaration(tree_numDecl(X,_),Env,Env,Scope):- lookup(X,Env,Scope,_), string_concat(X, " : Variable already declared", M), writeException(M), false.

eval_numberDeclaration(tree_numDecl(X),Env,EnvR,Scope):- not(lookup(X,Env,Scope,_)), update(X,0,Scope,Env,EnvR).
eval_numberDeclaration(tree_numDecl(X),Env,Env,Scope):- lookup(X,Env,Scope,_), string_concat(X, " : Variable already declared", M), writeException(M), false.
...
eval_booleanDeclaration(tree_boolDecl(X),Env,Env,Scope):- lookup(X,Env,Scope,_), string_concat(X, " : Variable already declared", M), writeException(M), false.
...
eval_stringDeclaration(tree_stringDecl(X,_),Env,Env,Scope):- lookup(X,Env,Scope,_), string_concat(X, " : Variable already declared", M), writeException(M), false.

eval_stringDeclaration(tree_stringDecl(X),Env,EnvR,Scope):- not(lookup(X,Env,Scope,_)), update(X,"",Scope,Env,EnvR).
eval_stringDeclaration(tree_stringDecl(X),Env,Env,Scope):- lookup(X,Env,Scope,_), string_concat(X, " : Variable already declared", M), writeException(M), false.
...
find_scope(Id,_,Scope,_):- Scope = -1,  string_concat(Id, " : Variable is not declared", M), writeException(M), false.

writeException(Val) :- string_concat("[EXP_##][Exception]:- ", Val, R), writeln(R).
